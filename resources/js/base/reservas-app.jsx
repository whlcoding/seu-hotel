// Reservas — list page with filters, table, drawer, pagination
const { useState, useMemo, useEffect, useRef } = React;
const { I } = window;

// ---------- Mock data (42 reservations) ----------
const ROOMS = {
...
const STATUS_LIST = ['confirmada', 'pendente', 'cancelada', 'realizada', 'no-show'];
const STATUS_LABEL = {
...
const CHANNELS = ['Recepção', 'Website', 'App', 'Booking.com', 'Expedia', 'Walk-in'];
...
function diffNights(a, b) { return Math.max(0, Math.round((b - a) / 86400000)); }
...
const RESERVATIONS = (() => {
  const r = rand(7);
  const out = [];
  const base = new Date(2026, 4, 1); // 1 May 2026
  const roomKeys = Object.keys(ROOMS);
  for (let i = 0; i < 42; i++) {
    const g = GUEST_POOL[Math.floor(r() * GUEST_POOL.length)];
    const room = roomKeys[Math.floor(r() * roomKeys.length)];
    const offset = Math.floor(r() * 35) - 8;
    const nights = 1 + Math.floor(r() * 5);
    const ci = new Date(base); ci.setDate(base.getDate() + offset);
    ...
    out.push({
      ...
      avatarColor: g[2],
      room,
      roomType: ROOMS[room].type,
      pricePerNight: ROOMS[room].price,
      checkin: ci,
      checkout: co,
      nights,
      guests,
      ...
      note: r() < 0.3 ? 'Hóspede solicita berço infantil.' : (r() < 0.5 ? 'Sem preferências.' : 'Quarto silencioso, longe do elevador.'),
    });
  }
  // sort by check-in desc by default
      ...
      <div className="chip-row">
        {items.map(it => (
          <button key={it.id}
            ...
            {it.dot && <span className="dot" style={{ background: it.dot }}></span>}
            {it.label}
  ...
  return (
    <div className="filter-group">
      ...
      <div className="date-range">
        <input type="date" className="input" value={from} onChange={e => onChange({ from: e.target.value, to })} aria-label="Data início" />
        <input type="date" className="input" value={to} onChange={e => onChange({ from, to: e.target.value })} aria-label="Data fim" />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="filter-group">
      ...
      <div className="input-wrap">
        ...
        <input className="input has-left has-right"
...
function ActionMenu({ row, onAction }) {
  const items = [
    ...
    { id: 'cancel', label: 'Cancelar reserva', icon: 'Logout', show: !['cancelada','realizada','no-show'].includes(row.status), danger: true },
  ];
  ...
  return (
    <div className="action-menu" onClick={e => e.stopPropagation()}>
      {visible.map((it, i) => {
        ...
        const sep = (i > 0 && it.danger && !visible[i - 1].danger);
        return (
          <React.Fragment key={it.id}>
            {sep && <div className="action-menu-sep"></div>}
            <button className={it.danger ? 'danger' : ''} onClick={() => onAction(it.id, row)}>
    ...
    <>
      ...
      <aside className="drawer" role="dialog" aria-label="Detalhes da reserva">
        <div className="drawer-head">
          <div>
            <h2 className="drawer-title">
              <span style={{ color: 'var(--ink-3)', fontWeight: 600, marginRight: 8 }} className="mono">#{row.id}</span>
              {row.guest}
            </h2>
            <div className="drawer-sub">
              <span className={`status ${row.status}`} style={{ marginRight: 8 }}>
                <span className="dot"></span>{STATUS_LABEL[row.status]}
              </span>
        ...
        <div className="drawer-body">
          <div className="drawer-section">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div className={`avatar ${row.avatarColor}`} style={{ width: 40, height: 40, flex: '0 0 40px', fontSize: 13 }}>{initials(row.guest)}</div>
              <div>
          ...
          <div className="drawer-section">
            ...
            <div className="kv-grid">
              <div className="kv-item">
                ...
                <span className="kv-v"><span className="mono">{row.room}</span> · {row.roomType}</span>
              </div>
              <div className="kv-item">
                <span className="kv-k">Hóspedes</span>
                <span className="kv-v">{row.guests} pessoa{row.guests === 1 ? '' : 's'}</span>
              </div>
              <div className="kv-item">
          ...
          <div className="drawer-section">
            ...
            <div className="drawer-total">
              ...
              <div className="drawer-total-row" style={{ marginTop: 8, paddingTop: 8, borderTop: '1px dashed var(--line-strong)' }}>
                <span>Status</span>
                <span>
                  {row.paid
                    ? <span className="status confirmada"><span className="dot"></span>Pago</span>
                    : <span className="status pendente"><span className="dot"></span>Aguardando</span>}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="drawer-section">
            ...
            <div className="timeline">
              <div className="tl-item">
                <div className="tl-dot"><I.Plus size={11} stroke={2.5} /></div>
              ...
              {row.status === 'confirmada' && (
                <div className="tl-item">
                  <div className="tl-dot green"><I.Check size={12} stroke={3} /></div>
                  <div className="tl-body">
                    Reserva confirmada pelo hóspede
                    <div className="tl-time">{fmtLong(new Date(row.created.getTime() + 86400000))} · 14:32</div>
                  </div>
                </div>
              )}
              {row.paid && (
                <div className="tl-item">
                  <div className="tl-dot green"><I.Cash size={12} stroke={2.5} /></div>
                  <div className="tl-body">
                    Pagamento confirmado · R$ {fmtBR(row.total + row.tax)}
                    <div className="tl-time">{fmtLong(new Date(row.created.getTime() + 2 * 86400000))} · 10:08</div>
                  </div>
                </div>
              )}
              {row.status === 'pendente' && (
                <div className="tl-item">
                  <div className="tl-dot orange"><I.Clock size={12} /></div>
                  <div className="tl-body">
        ...
        <div className="drawer-foot">
          ...
          {!['cancelada','realizada','no-show'].includes(row.status) && (
            <button className="btn" style={{ color: 'var(--red)', borderColor: 'var(--red-soft)' }}
...
function Pagination({ page, totalPages, onChange, total, start, end }) {
  ...
  if (max <= 7) {
    ...
    pages.push(1);
    if (cur > 2 + window_) pages.push('e1');
    for (let i = Math.max(2, cur - window_); i <= Math.min(max - 1, cur + window_); i++) pages.push(i);
    if (cur < max - 1 - window_) pages.push('e2');
    pages.push(max);
  }
  ...
  return (
    <div className="pagination">
      <div className="page-info">
        Mostrando <strong>{start}-{end}</strong> de <strong>{total}</strong> {total === 1 ? 'reserva' : 'reservas'}
      </div>
      <div className="page-nav">
        <button className="page-btn" disabled={page === 1} onClick={() => onChange(page - 1)} aria-label="Anterior">
          <I.ChevronLeft size={14} />
        </button>
        {pages.map((p, i) => p === 'e1' || p === 'e2'
          ? <span key={p} className="page-btn ellipsis">…</span>
          : <button key={p} className={`page-btn ${page === p ? 'active' : ''}`} onClick={() => onChange(p)}>{p}</button>
        )}
        <button className="page-btn" disabled={page === totalPages} onClick={() => onChange(page + 1)} aria-label="Próximo">
...
function App() {
  ...
  // Counts for status chips (apply other filters but not status)
  const baseFiltered = useMemo(() => {
    return RESERVATIONS.filter(r => {
      // date range
      if (dates.from && r.checkout < fromIso(dates.from)) return false;
      if (dates.to && r.checkin > fromIso(dates.to)) return false;
      // search
      if (search) {
  ...
  useEffect(() => {
    if (!menu) return;
    const h = (e) => {
      if (!e.target.closest('.action-menu') && !e.target.closest('.action-btn')) setMenu(null);
    };
  ...
  const filterCount = (status !== 'all' ? 1 : 0) + (dates.from || dates.to ? 1 : 0) + (search ? 1 : 0);
  ...
  const onRefresh = () => {
    ...
    setTimeout(() => { setRefreshing(false); showToast('Dados atualizados', 'success'); }, 900);
  };

  // Sort header helper
  const SortableTh = ({ k, children, ...props }) => (
    <th className={`sortable ${sortKey === k ? 'sorted' : ''}`} onClick={() => onSort(k)} {...props}>
      {children}
    ...
    <div className="page">
      ...
      <div className="page-head">
        <div>
          ...
          <div className="page-sub">{RESERVATIONS.length} reservas registradas no sistema · {counts.pendente || 0} pendente{(counts.pendente || 0) === 1 ? '' : 's'}</div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <a href="nova-reserva.html" className="btn primary" style={{ textDecoration: 'none' }}>
      ...
      <div className="filters">
        <StatusFilter value={status} onChange={(v) => { setStatus(v); setPage(1); }} counts={counts} />
        <DateRangeFilter from={dates.from} to={dates.to} onChange={(v) => { setDates(v); setPage(1); }} />
        <SearchFilter value={search} onChange={(v) => { setSearch(v); setPage(1); }} />
        <button className="btn ghost filter-clear" disabled={filterCount === 0} onClick={clearFilters}>
      ...
      <div className={`table-card ${density === 'compact' ? 'dense' : ''}`}>
        <div className="table-meta">
          <div className="left">
            <span><strong>{total}</strong> resultado{total === 1 ? '' : 's'}</span>
            {filterCount > 0 && <span className="bulk">filtros: {filterCount}</span>}
          </div>
        ...
        {total === 0 ? (
          ...
          <>
                  ...
                  {pageRows.map(r => (
                    <React.Fragment key={r.id}>
                      <tr className={expanded === r.id ? 'expanded' : ''}
                        ...
                        <td className="cell-id" style={{ paddingLeft: 18 }}>#{r.id}</td>
                        <td>
                          <div className="cell-guest">
                            <div className={`avatar ${r.avatarColor}`}>{initials(r.guest)}</div>
                            <div>
                              <div className="cell-name">{r.guest}</div>
                              <div className="cell-name-sub channel">via {r.channel}</div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="cell-date">
                          {fmtShort(r.checkin)}
                          <small style={{ textTransform: 'capitalize' }}>{weekdayShort(r.checkin)}</small>
                        </td>
                        <td className="cell-date">
                          {fmtShort(r.checkout)}
                          <small style={{ textTransform: 'capitalize' }}>{weekdayShort(r.checkout)}</small>
                        </td>
                        <td>
                          <span className={`status ${r.status}`}>
                            <span className="dot"></span>{STATUS_LABEL[r.status]}
                          </span>
                        </td>
                        <td className="cell-num">{r.nights}</td>
                        <td className="cell-total" style={{ textAlign: 'right' }}>R$ {fmtBR(r.total + r.tax)}</td>
                        <td className="cell-actions">
                          <div style={{ position: 'relative', display: 'inline-block' }}>
                            ...
                            {menu === r.id && <ActionMenu row={r} onAction={handleAction} />}
                          </div>
                        </td>
                      </tr>
                    </React.Fragment>
                ...
                <div key={r.id} className="res-card" onClick={() => setDrawerRow(r)}>
                  <div className="top">
                    <div className="guest-block">
                      ...
                      <div style={{ minWidth: 0 }}>
                        <div className="id-line">#{r.id} · {r.channel}</div>
                        <div style={{ fontWeight: 700, fontSize: 14 }}>{r.guest}</div>
                        <div style={{ fontSize: 11.5, color: 'var(--ink-3)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 200 }}>{r.email}</div>
                      </div>
                    </div>
                    <span className={`status ${r.status}`}>
                      <span className="dot"></span>{STATUS_LABEL[r.status]}
                  ...
                  <div className="grid2">
                    <div>
                      ...
                      <div className="v">{r.room} · <span style={{ fontFamily: 'Manrope', fontWeight: 500 }}>{r.roomType}</span></div>
                    </div>
                    <div>
                  ...
                  <div className="foot">
                    <span style={{ fontSize: 11.5, color: 'var(--ink-3)' }}>{r.nights} noite{r.nights === 1 ? '' : 's'}</span>
                    <div style={{ position: 'relative' }}>
                      <button className="action-btn" onClick={(e) => { e.stopPropagation(); setMenu(menu === r.id ? null : r.id); }} aria-label="Ações">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                          <circle cx="12" cy="5" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="12" cy="19" r="1.5"/>
                        </svg>
                      </button>
            ...
            <Pagination page={page} totalPages={totalPages} onChange={setPage} total={total} start={start} end={end} />
          </>
        )}
      </div>

      {/* Drawer */}
      {drawerRow && <Drawer row={drawerRow} onClose={() => setDrawerRow(null)} onAction={handleAction} />}
