// Equipe — team management with cards/table + schedule
const { useState, useMemo, useEffect, useRef } = React;
const { I } = window;

// ---------- Mock team data ----------
const ROLES = {
  recepcao:     { label: 'Recepção',     color: 'blue',   ink: 'var(--blue-ink)',   bg: 'var(--blue)' },
  housekeeping: { label: 'Housekeeping', color: 'green',  ink: 'var(--green-ink)',  bg: 'var(--green)' },
  gerente:      { label: 'Gerente',      color: 'purple', ink: 'var(--purple-ink)', bg: 'var(--purple)' },
  manutencao:   { label: 'Manutenção',   color: 'orange', ink: 'var(--orange-ink)', bg: 'var(--orange)' },
  cozinha:      { label: 'Cozinha',      color: 'red',    ink: 'var(--red-ink)',    bg: 'var(--red)' },
};
...
const SHIFT_ORDER = ['mat', 'vesp', 'noite', 'off', 'ferias'];

const TEAM_INIT = [
  {
    id: 1, name: 'Mariana Reis', email: 'mariana.reis@hotel.com', phone: '+55 11 99977-2211',
    cpf: '441.880.220-77', role: 'gerente', status: 'ativo', admission: '2023-03-15',
    shifts: ['mat', 'vesp'], salary: 6800, avatarColor: 'purple',
    schedule: ['mat','mat','mat','mat','mat','off','off'],
  },
  {
    id: 2, name: 'João da Silva', email: 'joao.silva@hotel.com', phone: '+55 85 99812-3456',
    cpf: '342.118.690-55', role: 'housekeeping', status: 'ativo', admission: '2025-01-10',
    shifts: ['mat'], salary: 2400, avatarColor: 'green',
    schedule: ['mat','vesp','mat','mat','vesp','off','off'],
  },
  {
    id: 3, name: 'Camila Souza', email: 'camila.souza@hotel.com', phone: '+55 21 99800-7733',
    cpf: '108.554.221-09', role: 'recepcao', status: 'ativo', admission: '2024-05-22',
    shifts: ['vesp'], salary: 2900, avatarColor: 'blue',
    schedule: ['vesp','mat','vesp','off','mat','mat','vesp'],
  },
  {
    id: 4, name: 'Pedro Henrique', email: 'pedro.henrique@hotel.com', phone: '+55 31 98800-1122',
    cpf: '903.221.114-44', role: 'manutencao', status: 'ativo', admission: '2024-09-01',
    shifts: ['mat', 'vesp'], salary: 3200, avatarColor: 'orange',
    schedule: ['off','mat','mat','mat','off','vesp','mat'],
  },
  {
    id: 5, name: 'Larissa Mendonça', email: 'lari.mend@hotel.com', phone: '+55 41 99655-3322',
    cpf: '551.224.788-90', role: 'housekeeping', status: 'ativo', admission: '2024-11-18',
    shifts: ['vesp'], salary: 2400, avatarColor: 'green',
    schedule: ['vesp','vesp','off','vesp','vesp','vesp','off'],
  },
  {
    id: 6, name: 'Rafael Lima', email: 'rafael.lima@hotel.com', phone: '+55 11 99811-4422',
    cpf: '775.331.004-12', role: 'recepcao', status: 'ferias', admission: '2023-07-04',
    shifts: ['mat'], salary: 3100, avatarColor: 'blue',
    schedule: ['ferias','ferias','ferias','ferias','ferias','ferias','ferias'],
  },
  {
    id: 7, name: 'Beatriz Castro', email: 'biacastro@hotel.com', phone: '+55 11 98744-5566',
    cpf: '215.667.890-12', role: 'cozinha', status: 'ativo', admission: '2025-02-20',
    shifts: ['mat', 'vesp'], salary: 2700, avatarColor: 'orange',
    schedule: ['mat','off','mat','vesp','vesp','mat','off'],
  },
  {
    id: 8, name: 'Eduardo Antunes', email: 'eduardo.antunes@hotel.com', phone: '+55 21 99112-9988',
    cpf: '903.221.118-77', role: 'manutencao', status: 'inativo', admission: '2022-08-12',
    shifts: ['noite'], salary: 2900, avatarColor: 'orange',
    schedule: ['off','off','off','off','off','off','off'],
  },
];

const WEEKDAYS = [
  { short: 'Seg', long: 'Segunda', weekend: false },
  { short: 'Ter', long: 'Terça', weekend: false },
  { short: 'Qua', long: 'Quarta', weekend: false },
  { short: 'Qui', long: 'Quinta', weekend: false },
  { short: 'Sex', long: 'Sexta', weekend: false },
...
function shiftLabel(s) {
  ...
  return shifts.map(x => SHIFTS[x]?.code).filter(Boolean).join(' · ');
}
function shiftRange(s) {
  const shifts = s.shifts || [];
  if (!shifts.length) return '';
  const ranges = shifts.map(x => SHIFTS[x]?.range).filter(Boolean);
  return ranges.join(' / ');
}
...
function StatCard({ tone, icon, value, label, delta }) {
  const IconC = I[icon];
  return (
    <div className="stat">
...
function StaffCard({ m, menuOpen, setMenuOpen, onAction }) {
  return (
    <div className="staff-card">
      ...
      <div className="staff-actions-row">
        <button className="btn sm" onClick={() => onAction('edit', m)}>
          <I.Tools size={13} /> Editar
        </button>
        <button className="btn sm" onClick={() => onAction('schedule', m)}>
          <I.Calendar size={13} /> Escala
        </button>
      </div>
    </div>
  );
...
function StaffTable({ rows, menuOpen, setMenuOpen, onAction }) {
  return (
...
function MemberModal({ initial, onClose, onSave }) {
  ...
  const [form, setForm] = useState(initial || {
    name: '', email: '', phone: '', cpf: '', admission: '',
    role: 'housekeeping', status: 'ativo', shifts: ['mat'], salary: '',
    avatarColor: 'blue',
    schedule: ['mat','mat','mat','mat','mat','off','off'],
  });
  ...
  const validate = () => {
    ...
    if (!form.shifts || !form.shifts.length) e.shifts = 'Selecione ao menos 1 turno';
    setErrors(e);
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-label={isEdit ? 'Editar colaborador' : 'Adicionar colaborador'}>
          ...
          <div className="form-grid">
            <div className="form-col">
              <div className={`field ${errors.name ? 'error' : ''}`}>
                ...
                <div className="error-msg"><I.Warning size={11} /> {errors.name}</div>
              </div>
              <div className={`field ${errors.email ? 'error' : ''}`}>
                <label className="label">Email <span className="req">*</span></label>
                <input type="email" className="input" placeholder="nome@hotel.com"
                       value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
                <div className="error-msg"><I.Warning size={11} /> {errors.email}</div>
              </div>
              <div className={`field ${errors.phone ? 'error' : ''}`}>
                ...
                <div className="error-msg"><I.Warning size={11} /> {errors.phone}</div>
              </div>
              <div className={`field ${errors.cpf ? 'error' : ''}`}>
                <label className="label">CPF <span className="req">*</span></label>
                <input className="input mono" placeholder="000.000.000-00" inputMode="numeric"
                       value={form.cpf}
                       onChange={(e) => setForm({ ...form, cpf: fmtCpf(e.target.value) })} />
                <div className="error-msg"><I.Warning size={11} /> {errors.cpf}</div>
              </div>
              <div className={`field ${errors.admission ? 'error' : ''}`}>
                ...
                <div className="error-msg"><I.Warning size={11} /> {errors.admission}</div>
              </div>
            </div>
            ...
            <div className="form-col">
                ...
                <div className="radio-grid" style={{ gridTemplateColumns: 'repeat(2, 1fr)' }}>
                  {Object.entries(ROLES).map(([key, r]) => (
                    <button key={key} type="button"
                            className={`radio-btn ${form.role === key ? 'active' : ''}`}
              ...
              <div className="field">
                ...
                <div className="radio-grid">
                  {['ativo','ferias','inativo'].map(s => (
                    <button key={s} type="button"
                            className={`radio-btn ${form.status === s ? 'active' : ''}`}
              ...
              <div className={`field ${errors.shifts ? 'error' : ''}`}>
                ...
                <div className="check-grid">
                  {['mat','vesp','noite'].map(s => (
                    <button key={s} type="button"
                            className={`check-btn ${form.shifts.includes(s) ? 'active' : ''}`}
                ...
                <div className="error-msg" style={{ display: errors.shifts ? 'flex' : 'none' }}>
                  <I.Warning size={11} /> {errors.shifts}
                </div>
              </div>
        ...
        <div className="modal-foot">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {isEdit && (
              <button className="btn danger" onClick={() => onSave({ ...form, _delete: true })}>
                Remover
              </button>
            )}
...
function ScheduleView({ team, onChange }) {
  const [popover, setPopover] = useState(null); // { id, day, x, y }
  const popRef = useRef(null);

  useEffect(() => {
    if (!popover) return;
    const h = (e) => {
      if (popRef.current && !popRef.current.contains(e.target) && !e.target.closest('.shift-cell')) setPopover(null);
    };
  ...
  const counts = useMemo(() => {
    ...
    activeTeam.forEach(m => {
      m.schedule.forEach(s => { c[s] = (c[s] || 0) + 1; });
    });
    return c;
  }, [activeTeam]);
  ...
  const onCellClick = (e, mid, dayIdx) => {
            ...
            <button key={s} onClick={() => setShift(s)}>
              <span className="sw"
                    style={{
                      background:
                        s === 'mat' ? 'var(--blue-soft)' :
                        s === 'vesp' ? 'var(--orange-soft)' :
                        s === 'noite' ? 'var(--purple-soft)' :
                        s === 'ferias' ? 'var(--green-soft)' : '#f1f0ea',
                      border: `1px solid ${
                        s === 'mat' ? 'var(--blue)' :
                        s === 'vesp' ? 'var(--orange)' :
                        s === 'noite' ? 'var(--purple)' :
                        s === 'ferias' ? 'var(--green)' : 'var(--line-strong)'}`
                    }}></span>
...
function App() {
  const [tab, setTab] = useState('team'); // 'team' | 'schedule'
  ...
  useEffect(() => {
    if (!menuOpen) return;
    const h = (e) => {
      if (!e.target.closest('.action-menu') && !e.target.closest('.action-btn')) setMenuOpen(null);
    };
    window.addEventListener('mousedown', h);
    return () => window.removeEventListener('mousedown', h);
  }, [menuOpen]);
    ...
    switch (action) {
      ...
      case 'deactivate':
        if (window.confirm(`Desativar ${row.name}? O acesso ao sistema será revogado.`)) {
          ...
          showToast(`${row.name} foi desativada`, 'warn');
        }
        break;
      case 'activate':
        setTeam(team.map(m => m.id === row.id ? { ...m, status: 'ativo' } : m));
        showToast(`${row.name} foi reativada`, 'success');
        break;
      default:
    }
  };
  ...
  const handleSave = (form) => {
    if (form._delete) {
      ...
      setModal(null);
      showToast(`${form.name} removida da equipe`, 'warn');
      return;
    }
    if (form.id) {
      setTeam(team.map(m => m.id === form.id ? { ...m, ...form } : m));
      showToast(`Dados de ${form.name} atualizados`, 'success');
    } else {
      const id = Math.max(...team.map(m => m.id)) + 1;
      const colors = ['blue','green','purple','orange',''];
      setTeam([...team, { ...form, id, avatarColor: colors[Math.floor(Math.random() * colors.length)] }]);
      showToast(`${form.name} adicionada à equipe`, 'success');
    }
    setModal(null);
  };
  ...
  const updateSchedule = (memberId, dayIdx, newShift) => {
    setTeam(team.map(m => {
  ...
  return (
    <div className="page">
      ...
      {tab === 'team' && (
        <>
          ...
          <div className="filters">
            ...
            <select className="select" value={role} onChange={(e) => setRole(e.target.value)} aria-label="Função">
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              {filterCount > 0 && (
                <button className="btn ghost sm" onClick={clearFilters} title="Limpar filtros">
                  Limpar ({filterCount})
                </button>
              )}
      ...
      {/* Modal */}
      {modal !== null && (
        <MemberModal initial={modal && modal.id ? modal : null}
